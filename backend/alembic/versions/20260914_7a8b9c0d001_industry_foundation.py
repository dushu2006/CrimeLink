...
def upgrade() -> None:
    ...
    if "evidence_custody_events" not in existing_tables:
        op.create_table(
        ...
        existing_indexes = {idx["name"] for idx in inspector.get_indexes("evidence_custody_events")}
        if "ix_custody_evidence" not in existing_indexes:
            op.create_index("ix_custody_evidence", "evidence_custody_events", ["evidence_id"])
        if "ix_custody_case" not in existing_indexes:
            op.create_index("ix_custody_case", "evidence_custody_events", ["case_id"])
        if "ix_custody_case_time" not in existing_indexes:
            op.create_index("ix_custody_case_time", "evidence_custody_events", ["case_id", "created_at"])

    if "investigation_tasks" not in existing_tables:
        op.create_table(
            "investigation_tasks",
            sa.Column("id", sa.String(36), primary_key=True),
            sa.Column("case_id", sa.String(36), nullable=False),
        ...
        existing_indexes = {idx["name"] for idx in inspector.get_indexes("investigation_tasks")}
        if "ix_tasks_case" not in existing_indexes:
            op.create_index("ix_tasks_case", "investigation_tasks", ["case_id"])

    if "investigator_notes" not in existing_tables:
        op.create_table(
            "investigator_notes",
            sa.Column("id", sa.String(36), primary_key=True),
            sa.Column("case_id", sa.String(36), nullable=False),
        ...
        existing_indexes = {idx["name"] for idx in inspector.get_indexes("investigator_notes")}
        if "ix_notes_case" not in existing_indexes:
            op.create_index("ix_notes_case", "investigator_notes", ["case_id"])
    ...
    if "hypotheses" not in existing_tables:
        ...
        if "ix_hypotheses_case" not in existing_indexes:
            op.create_index("ix_hypotheses_case", "hypotheses", ["case_id"])
    ...
    if "contradictions" not in existing_tables:
        ...
        existing_indexes = {idx["name"] for idx in inspector.get_indexes("contradictions")}
        if "ix_contradictions_case" not in existing_indexes:
            op.create_index("ix_contradictions_case", "contradictions", ["case_id"])
        if "ix_contradictions_subject" not in existing_indexes:
            op.create_index("ix_contradictions_subject", "contradictions", ["subject_key"])
    ...
    if "claims" not in existing_tables:
        ...
        if "ix_claims_case" not in existing_indexes:
            op.create_index("ix_claims_case", "claims", ["case_id"])
        if "ix_claims_subject" not in existing_indexes:
            op.create_index("ix_claims_subject", "claims", ["subject"])
    ...
    if "approval_records" not in existing_tables:
        ...
        existing_indexes = {idx["name"] for idx in inspector.get_indexes("approval_records")}
        if "ix_approvals_case" not in existing_indexes:
            op.create_index("ix_approvals_case", "approval_records", ["case_id"])
        if "ix_approvals_object" not in existing_indexes:
            op.create_index("ix_approvals_object", "approval_records", ["object_id"])
    ...
    if "model_registry" not in existing_tables:
        ...
        existing_indexes = {idx["name"] for idx in inspector.get_indexes("model_registry")}
        if "ix_model_registry_role" not in existing_indexes:
            op.create_index("ix_model_registry_role", "model_registry", ["role"])

    if "investigation_reports" not in existing_tables:
        op.create_table(
        ...
        existing_indexes = {idx["name"] for idx in inspector.get_indexes("investigation_reports")}
        if "ix_reports_case" not in existing_indexes:
            op.create_index("ix_reports_case", "investigation_reports", ["case_id"])
...
def downgrade() -> None:
    ...
    for table in (
        ...
        if table in existing_tables:
            op.drop_table(table)
    if "case_documents" in existing_tables:
        existing_cols = {c["name"] for c in inspector.get_columns("case_documents")}
    ...
    if "users" in existing_tables:
        ...
        if "max_classification" in existing_cols:

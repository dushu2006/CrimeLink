...
const DEFAULT_ACTIONS: Action[] = [
  { icon: "group", title: "Review people", description: "Identify people connected to this investigation.", to: "/people" },
  { icon: "hub", title: "Review relationships", description: "Understand how the people and records connect.", to: "/relationships" },
  { icon: "description", title: "Review evidence", description: "Inspect the source records behind the findings.", to: "/evidence" },
  { icon: "schedule", title: "Examine timeline", description: "Reconstruct what happened and when.", to: "/timeline" },
];
      ...
      <div className="what-next-grid">
        {actions.map((action) => (
          <button key={action.title} className="what-next-action" onClick={() => navigate(action.to)}>
            <span className="material-symbols-outlined what-next-icon" aria-hidden="true">{action.icon}</span>
